...
if Username == "Manage Profiles":
	...
	if xbmc.getSkinDir() == "Manage Profiles Skin":
		...
		IDs=80
		for _ in range(11):
			xbmc.executebuiltin('Skin.Reset(CustomViewtype_id_'+str(IDs)+')')
			IDs+=1
		for images in sorted(os.listdir(Path)):
			if images.startswith("CustomViewtype_id_") and images.endswith(".jpg"):
				file=os.path.splitext(images)[0]
				valid=bool(re.search(r'\d', file))
				if valid: xbmc.executebuiltin('Skin.SetBool('+file+')')	
