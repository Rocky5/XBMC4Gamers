...
CStdString patchedpath;
CStdString patchedwspath;
CStdString patchedxhdpath;
CStdString patched720path;
	...
	if (item && !item->GetPropertyBOOL("pluginreplacecontextitems"))
	{
		...
		else
		{
			if (item->IsXBE() || item->IsShortCut())
			{
				...
				{
					patchedpath = (patchedxbepath+"default_p.xbe");
				}
				if (CFile::Exists(patchedpath))
				{
					buttons.Add(CONTEXT_BUTTON_PATCHEDXBE, "Run Patched XBE");         // Patched XBE
				}
				
				if (CFile::Exists(patchedxbepath+"defaultws.xbe"))
				{
					patchedwspath = (patchedxbepath+"defaultws.xbe");
				}
				if (CFile::Exists(patchedwspath))
				{
					buttons.Add(CONTEXT_BUTTON_PATCHEDWSXBE, "Run Widescreen XBE");         // Patched XBE
				}
				
				if (CFile::Exists(patchedxbepath+"defaultxhd.xbe"))
				{
					patchedxhdpath = (patchedxbepath+"defaultxhd.xbe");
				}
				if (CFile::Exists(patchedxhdpath))
				{
					buttons.Add(CONTEXT_BUTTON_PATCHEDXHDXBE, "Run XHD XBE");         // Patched XBE
				}
				
				if (CFile::Exists(patchedxbepath+"default720p.xbe"))
				{
					patched720path = (patchedxbepath+"default720p.xbe");
				}
				if (CFile::Exists(patched720path))
				{
					buttons.Add(CONTEXT_BUTTON_PATCHED720XBE, "Run 720p XBE");         // Patched XBE
				}
...
bool CGUIWindowPrograms::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
	...
	case CONTEXT_BUTTON_PATCHEDXBE:
		{
			CBuiltins::Execute("runxbe("+patchedpath+")");
			return true;
		}
	case CONTEXT_BUTTON_PATCHEDWSXBE:
		{
			CBuiltins::Execute("runxbe("+patchedwspath+")");
			return true;
		}
	case CONTEXT_BUTTON_PATCHEDXHDXBE:
		{
			CBuiltins::Execute("runxbe("+patchedxhdpath+")");
			return true;
		}
	case CONTEXT_BUTTON_PATCHED720XBE:
		{
			CBuiltins::Execute("runxbe("+patched720path+")");
			return true;
		}
...
bool CGUIWindowPrograms::GetDirectory(const CStdString &strDirectory, CFileItemList &items)
{
	...
	for (int i = 0; i < items.Size(); i++)
	{
		...
		if (item->IsXBE())
		{
			...
			else if (URIUtils::GetFileName(item->GetPath()).Equals("defaultxws.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}   
